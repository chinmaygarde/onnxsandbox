...
BufferView::BufferView(DeviceBufferVK* buffer, Range range)
    : buffer_(), raw_buffer_(buffer), range_(range) {}

BufferView::BufferView(std::shared_ptr<const DeviceBufferVK> buffer,
                       Range range)
    : buffer_(std::move(buffer)), raw_buffer_(nullptr), range_(range) {}

const DeviceBufferVK* BufferView::GetBuffer() const {
  return raw_buffer_ ? raw_buffer_ : buffer_.get();
}

std::shared_ptr<const DeviceBufferVK> BufferView::TakeBuffer() {
  if (buffer_) {
